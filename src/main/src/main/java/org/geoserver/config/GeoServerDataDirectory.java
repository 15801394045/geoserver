 ...
 *
 * <p>对GeoServer数据目录的文件或资源访问。除了路径之外，目录对象（如工作区或FeatureTypeInfo）还可用于定位资源。
 *
 * <p>Example usage:
 *
 * <pre>
...
public class GeoServerDataDirectory {
    ...
    /** Creates the data directory specifying the base directory. 创建指定基本目录的数据目录。 */
    public GeoServerDataDirectory(File baseDirectory) {
        this(new GeoServerResourceLoader(baseDirectory));
    }

    /** Returns the underlying resource loader. 返回基础资源加载程序。 */
    ...
    /** The root of the data directory. 数据目录的根目录。 */
    public File root() {
     ...
     * will be created. 返回一个目录，该目录位于{@link #root()}目录下，如果目录不存在，它将被创建。
     *
     ...
     * returned.
     *
     * <p>返回一个文件，在{@link #root()} 目录下，如果文件不存在，返回null。
     */
     ...
     * exist, null is returned. 返回包含空间数据文件的目录的根目录，如果目录确实存在，则返回NULL。
     *
